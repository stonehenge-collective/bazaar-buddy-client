...
class BazaarMonitor:
    ...
    def __init__(self, logger):
        self.overlay = Overlay("Waiting for The Bazaar to start…")
        self.logger = logger
        self.controller = CaptureController(self.overlay, self.logger)
        self.system_handler = get_system_handler()
        self.updater = BazaarUpdater(self.overlay, self.logger)
