...
class BazaarBuddy:
    ...
    def _attempt_start(self):
        ...
        self.logger.info(f"[{threading.current_thread().name}] Attempting to find the Bazaar process…")
        ...
        if not bazaar_proc:
            self.logger.info(f"[{threading.current_thread().name}] Could not find process with name: {process_name}")
            return

        self.logger.info(f"[{threading.current_thread().name}] Found Bazaar process with PID: {bazaar_proc.pid}")
        window_handle = self.system_handler.find_process_main_window_handle(bazaar_proc.pid)

        if not window_handle:
            self.logger.info(f"[{threading.current_thread().name}] Could not find main window handle for process")
            return

        self.logger.info(f"[{threading.current_thread().name}] Found window handle: {window_handle}")

        self.overlay.set_message("Bazaar process found, watching…")

        self.logger.info(f"[{threading.current_thread().name}] Bazaar process found")

        self.logger.info(f"[{threading.current_thread().name}] stopping trying to _attempt_start")
        self.timer_worker.disconnect(self.attempt_start_connection)

        self.logger.info(f"[{threading.current_thread().name}] Starting capture worker…")
